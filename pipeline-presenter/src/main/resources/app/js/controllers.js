...
//    });
    ...
    $scope.isPipelineRunNotAccepted = function (build) {
        return $.grep(build.steps, function (step) {
            return $scope.isStepNotAccepted(step);
        }).length > 0;
    ...
    $scope.isPipelineRunFailed = function (build) {
        return $.grep(build.steps, function (step) {
            return $scope.isStepFailed(step);
        }).length > 0;
    };

    $scope.isPipelineRunActive = function (build) {
        return $.grep(build.steps, function (step) {
            return $scope.isStepActive(step);
        }).length > 0;
    };

    $scope.isPipelineRunPending = function (build) {
        return !$scope.isPipelineRunSuccess(build) && !$scope.isPipelineRunActive(build) && !$scope.isPipelineRunFailed(build) && !$scope.isPipelineRunNotAccepted(build);
    };
    ...
    $scope.findPipeline = function (name) {
        return $.grep($scope.pipelines, function (pipeline) {
